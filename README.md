...
**实测（12 只 / 221 条信号 / 5 日窗口）**：18 个组合里「配合组显著更好」**0 个**、「显著更差」**0 个**。
唯一擦边的是 `W1 × 第一类买点`：差值 **+2.27%**，CI `[+0.01%, +4.54%]` 刚好不跨 0 ——
但对照组只有 8 条，样本不足，**不能据此下结论**。
→ **「级别共振」在这个样本里没有得到支持。**

---

除此之外还有一套**完全离线**的单元测试（不联网、不需要本地行情数据，2.4 秒跑完，78 项）：
