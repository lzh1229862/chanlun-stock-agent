...
- 信号分布 **上行 333 / 震荡 259 / 下行 37** → **下行只占 5.5%，样本里几乎没有熊市**
- **买点在任何状态下都没有显著 alpha** —— 之前的「买点超额」大部分是 beta
- **卖点的正 alpha 稳定**：震荡 5 日 +1.15%、上行 10 日 +1.37%、上行 20 日 +2.28%
- **一个被指数基准当场揭穿的例子**：买点/下行/5 日，「超额(同股)」+1.18% 看着不错，
  但换成指数基准后 alpha 只有 **+0.21%** —— 那段时间指数自己就涨了 1.65%
