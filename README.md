...
完整决策记录见 [`docs/技术决策记录.md`](docs/技术决策记录.md)，共 35 条。
...
| 032 | B轮：交易成本敏感性 | 打分选出高波动（1.6x）但非低流动性；**score=2 能扛到 3% 往返成本（现行假设的 15 倍）**；分层滑点不改变结论；**中枢宽度 = 波动率代理**（corr +0.435） |
| 033 | B轮：多重比较台账与 FDR | 全项目 **175 次检验**；114 次有 CI，全局 BH 后 **78 次是发现**（期望运气仅 5.7）；⚠️ **H3 距中枢不过 FDR，应降级为线索** |
| 034 | B轮：信号失效提醒 / 本股打分历史 / 默认白天 | B7 对比历史日报与当前库（实测 0 条失效，是安全网）；B8 单股本股打分表现；**默认白天要改三处**（漏了 config.toml 会一半白一半黑） |
| 035 | B轮：幸存者偏差的正面检验 | 退市股 252/252 全取到；**退市股信号超额显著更高（修正因子后 +0.46% / +0.48% / +0.78%），与预期相反**；排除了 ST 窗口这个 confound -> 偏差没有让核心数字虚高 |
