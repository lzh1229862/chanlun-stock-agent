...
完整决策记录见 [`docs/技术决策记录.md`](docs/技术决策记录.md)，共 23 条。
...
| 022 | 全市场买点扫描器 | 独立轻量路径 + 独立表；窗口按确认日；原始命中 1000+ 条须靠筛选收窄 |
| 023 | 第2轮假设 + 排序打分 | 5 条新假设全灭；**中枢宽度八档阈值全显著（最稳健因子）**；打分三窗口全过但属探索性 |
| 024 | 打分的时间样本外验证 | 训练 2015-2020 / 检验 2021-2026 → **3/3 窗口通过**；⭐ **衰减一致 30~33%，样本内数字应打七折** |
| 022 | 全市场买点扫描器 | 独立轻量路径 + 独立表；窗口按确认日；**原始命中 1000+ 条（一买在下跌周聚集）**，须靠筛选收窄 |
| 023 | 第2轮假设 + 排序打分 | 5 条新假设全灭；**中枢宽度八档阈值全显著（最稳健因子）**；打分三窗口全过但属**探索性** |
