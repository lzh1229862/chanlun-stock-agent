...
UP, DOWN = "向上", "向下"
MIN_BI_LEN = 5
# 2026-09-20（T14-1）：从 500 提到 5000。历史扩到 11 年后池内最大笔数已达 432，
# 距 500 只剩 14% 余量；而 czsc 的 max_bi_num 是**静默截断**（超了不报错，直接丢掉
# 早期的笔、首笔日期会漂移），13 年以上必然撞线。实测 500 vs 9999 在当前数据上
# 结果完全一致，所以这次提高对本轮数据是 no-op，只是拆掉这个雷。
# 另见 ADR-005 补记：min_bi_len 是分档开关，max_bi_num 是静默截断 —— 两个都不能乱动。
MAX_BI_NUM = 5000
MAX_ZS_BIS = 9
