...
| 第 17 轮 | H3 接进产品（距中枢提示） | `structure_gap.py` / ADR-021 补记（T17） |
...
| 第 26 轮 | 日更性能：26.8 小时 -> 12 分钟（因子缓存 + 批量行情） | batch_quote.py（ADR-028） |
| 第 27 轮 | B1 既有结论的时间样本外体检（9 条 8 成立） | verify_claims.py（ADR-029） |
| 第 28 轮 | B2 涨跌停可执行性（入场顺延） | backtest.py（ADR-030） |
| 第 29 轮 | B3 上市时间点 / 样本构成 / 衰减真正的来源 | verify_claims 延伸（ADR-031） |
...
- **打分的样本外转正**（T20 / ADR-024 / `verify_score.py`）：
  - 动机：ADR-023 的分半验证**不能替代时间样本外**（同一批数据、同一套阈值；阈值 0.12/10 是在全样本上选的）
  - 做法：`prereg` 先落盘假设/阈值/期间/通过标准 → `test` 只读它做验证；
    **训练期 2015-2020（5097 条）/ 检验期 2021-2026（7240 条）**，基准在各期自己跨度内重算
  - **结果：检验期 3/3 窗口通过**（CI 不跨 0 + 分半一致 + 单调，三条同时满足）
  - ⭐ **最有价值的数字：三窗口衰减一致在 −32% / −33% / −30%** —— 这么一致不可能是噪声，
    说明**样本内数字要比真实预期打七折**。前面所有 ADR 的数字都是全样本的，都该按此折算
  - 次级分析：**只用训练期重选阈值** → 中枢宽度 0.100（预注册 0.12）、距中枢 12（预注册 10），
...
- [ ] 竞品可借鉴但未做：流通盘分档 / 中枢区间展示 / 行业中性化体检 / 个股走势状态
